...
class FileShareManifest : public ContentDescription{
public:
  ...
  {
      FileShareManifest* manifest = new FileShareManifest(*this);

	  ItemList::const_iterator iter = this->items_.begin();

	  while(iter != this->items_.end())
	  {
		  manifest->items_.push_back(*iter);
		  iter++;
	  }
...
public:
  ...
  sigslot::signal4<std::string, int, int, talk_base::HttpServerTransaction*> SignalResampleImage;
...
private:
  ...
  // Session Signals
  void OnSessionState(cricket::BaseSession* session, cricket::BaseSession::State state);
  void OnSessionInfoMessage(cricket::Session* session,
                            const buzz::XmlElement* els);
  void OnSessionChannelGone(cricket::BaseSession* session,

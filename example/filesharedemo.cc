...
class FileShareClient : public sigslot::has_slots<>, public talk_base::MessageHandler {
 ...
 private:
  ...
  void OnStatusUpdate(const buzz::PresenceStatus &status) {
	...
	share->Share(status.jid().BareJid(), const_cast<cricket::FileShareManifest*>(manifest_));
  ...
  void OnResampleImage(std::string path, int width, int height, talk_base::HttpServerTransaction *trans) {  
  ...
  void OnSignon() {
    ...
    buzz::PresenceReceiveTask *presence_push_ = new buzz::PresenceReceiveTask(xmpp_client_);
    presence_push_->PresenceUpdate.connect(this, &FileShareClient::OnStatusUpdate);
    presence_push_->Start();
    
    buzz::PresenceStatus my_status;
    my_status.set_jid(xmpp_client_->jid());
    my_status.set_available(true);
    my_status.set_show(buzz::PresenceStatus::SHOW_ONLINE);
	...
	session_manager_->AddClient(cricket::NS_JINGLE, file_share_session_client_.get());
	session_manager_->AddClient(buzz::NS_FILESHARE_DECRIPTION, file_share_session_client_.get());
  }
...
*/
  ...
  xcs.set_server(talk_base::SocketAddress("192.168.1.101", 5222));
  ...
  for (;i < 1;i++) {
    ...
    if (talk_base::Filesystem::IsFolder(std::string("E:\\project\\IM\\Data\\"))) {
      manifest->AddFolder("E:\\project\\IM\\Data\\", get_dir_size("E:\\project\\IM\\Data\\"));
    } else {
      size_t size = 0;
      talk_base::Filesystem::GetFileSize(std::string("E:\\project\\IM\\Data\\test.txt"), &size);
      manifest->AddFile("E:\\project\\IM\\Data\\test.txt", size);
    }
  }
  buzz::Jid j;
  j = buzz::Jid("zyf@miaomiao");
//   if (send_mode)
//     j = buzz::Jid("yym@miaomiao");
//   else
//     j = buzz::JID_EMPTY;
  ...
  pump.DoLogin(xcs, new buzz::XmppSocket(buzz::TLS_ENABLED), NULL);
  main_thread.Run();
